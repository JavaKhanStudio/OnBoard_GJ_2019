...
{
	ListStyle style;
	final Array<GameItem> selected = new Array<GameItem>();
	ArraySelection<GameItem> selection ;
	...
	{
		selection = new ArraySelection<GameItem>(selected) 
		{
			@Override
	...
	public ArraySelection<GameItem> getSelection()
	{return selection;}
	...
	public GameItem getItemAt(float y)
	{
		int index = getItemIndexAt(y);
		if (index == -1)
			return null;
	...
	public void setItems(GameItem... newItems)
	{
		if (newItems == null)
	...
	public void setItems(Collection<GameItem> newItems)
	{
		Array myItems = new Array<GameItem>() ; 
		for(GameItem item : newItems)
		{
			myItems.add(item);
		}
		setItems(myItems) ; 
	...
	protected void drawItem(Batch batch, BitmapFont font, int index, GameItem item, float x, float y, float width)
	{
		batch.draw(item.objectTexture, 
				x , y + decalY, 
