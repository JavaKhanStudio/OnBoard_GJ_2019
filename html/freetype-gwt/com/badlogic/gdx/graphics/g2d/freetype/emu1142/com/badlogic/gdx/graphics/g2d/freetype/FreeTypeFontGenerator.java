/* On Board (r137): libGDX 1.14.2's FreeTypeFontGenerator with the three changes gdx-freetype-gwt
 * 1.9.10.1 made to 1.9.10's for the browser (the shadow pixmap is a FreeTypePixmap, whose pixels are
 * read and written back by hand). The port's own copy is 1.9.10's and no longer compiles against
 * libGDX 1.14.2's PixmapPacker. Take libGDX's file again and redo those three when gdxVersion moves. */
/*******************************************************************************
	...
	protected @Null Glyph createGlyph (char c, FreeTypeBitmapFontData data, FreeTypeFontParameter parameter, Stroker stroker,
		...
		if (mainBitmap.getWidth() != 0 && mainBitmap.getRows() != 0) {
			...
			if (parameter.shadowOffsetX != 0 || parameter.shadowOffsetY != 0) {
				...
				// On Board (r137): FreeTypePixmap, as gdx-freetype-gwt 1.9.10.1 does, since a browser has no Gdx2DPixmap
